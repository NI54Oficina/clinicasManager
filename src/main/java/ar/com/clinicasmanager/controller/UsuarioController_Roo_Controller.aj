...
import ar.com.clinicasmanager.controller.UsuarioController;
...
privileged aspect UsuarioController_Roo_Controller {
    ...
    @Autowired
    UsuarioDAO UsuarioController.usuarioDAO;
    
    @Autowired
    GrupoDAO UsuarioController.grupoDAO;
    ...
    public String UsuarioController.show(@PathVariable("id") Long id, Model uiModel) {
        uiModel.addAttribute("usuario", usuarioDAO.findOne(id));
    ...
    public String UsuarioController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("usuarios", usuarioDAO.findAll(new org.springframework.data.domain.PageRequest(firstResult / sizeNo, sizeNo)).getContent());
            float nrOfPages = (float) usuarioDAO.count() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("usuarios", usuarioDAO.findAll());
        }
    ...
    public String UsuarioController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, usuarioDAO.findOne(id));
        return "usuarios/update";
    }
